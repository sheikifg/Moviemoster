	...
	<div class="dt_owl_carousel owl-theme owl-carousel slider" data-owl-options='{
		...
		<?php
			...
			foreach ( $corpiva_slider_option as $item ) {
				...
				$button2 = ! empty( $item->button_second) ? apply_filters( 'corpiva_translate_single_string', $item->button_second,'slider section' ) : '';
				$link2 = ! empty( $item->link2 ) ? apply_filters( 'corpiva_translate_single_string', $item->link2, 'slider section' ) : '';
				$link3 = ! empty( $item->link3 ) ? apply_filters( 'corpiva_translate_single_string', $item->link3, 'slider section' ) : '';
				$subtitle3 = ! empty( $item->subtitle3 ) ? apply_filters( 'corpiva_translate_single_string', $item->subtitle3, 'slider section' ) : '';
				$icon = ! empty( $item->icon_value ) ? apply_filters( 'corpiva_translate_single_string', $item->icon_value, 'slider section' ) : '';
										...
										<div class="dt_slider-content">
											...
											<div class="dt_btn-group">
												...
												<?php if ( ! empty( $button2 )) : ?>
													<a href="<?php echo esc_url($link2); ?>" class="dt-btn dt-btn-white dt-btn-border"><?php echo wp_kses( html_entity_decode( $button2 ), $allowed_html ); ?></a>
												<?php endif; ?>	
												
												<?php if ( ! empty( $icon ) || ! empty( $subtitle3 )) : ?>
													<a href="<?php echo esc_url($link3); ?>" class="dt-btn-play-one dt_lightbox_img" data-caption="<?php esc_attr_e('Play Video','desert-companion'); ?>"><i class="<?php echo esc_attr($icon); ?>" aria-hidden="true"></i> <span><?php echo wp_kses( html_entity_decode( $subtitle3 ), $allowed_html ); ?></span></a>
												<?php endif; ?>	
											</div>
										</div>
									</div>
